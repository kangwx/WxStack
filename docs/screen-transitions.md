...
- StackWorld3D.setPresentationOpacity 只控制积木、碎片和描边的可视节点。使用缓存的 builtin-unlit transparent 材质；完全淡出后才重置积木和镜头，结束后换回原不透明材质。主题背景、碰撞体和刚体不参与淡出。
...
- 自动化：两段过渡的起始/中间/结束状态、重置前完全隐藏、重复输入、菜单与游戏切换、后台立即收敛、缩放时恢复 Widget、减少动态效果、六种主题的压暗上限、材质缓存及销毁、碎片/描边与背景隔离。测试使用 Cocos test double，不代表 GPU 或 Android 实机验证。
...
- 尚无 Android WebView 实机连接；实际遥控器、WebView 帧率及全部主题的实机视觉验收仍需设备测试。
