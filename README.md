...
- 初始 100 金币、完美次数奖励与皮肤购买；首页完美测试开关
...
- 结束页提供重新开始和返回首页；结束/暂停返回首页采用 0.20 秒淡出、0.28 秒淡入，过渡期间拦截输入，减少动态效果时直接切换。
...
`assets/scripts/StackGame.ts` 管理玩法、UI、计分、连击、金币、主题和音效；`assets/scripts/StackWorld3D.ts` 管理相机、网格、材质及物理碰撞。HUD 通过 `Widget` 与 `SafeArea` 适配屏幕。
...
5. 切换皮肤，并检查窄屏和宽屏：图片铺满、文字可读、自然主题纹理保留。
