...
用 Cocos Creator 3.8.8 制作的移动端 3D 休闲堆塔游戏。方块沿 X/Z 两轴交替移动，点击后短距离下落，按与目标塔层的实际重合范围保留方块；切片以独立刚体坠落，精准对齐会累计完美连击。

## 已实现

- 触屏、鼠标、空格、手柄 A 落块；P / Esc / Options 暂停，暂停菜单支持继续、重新开始、返回首页
- X/Z 两轴交替移动：新方块依次从左上朝右下、右上朝左下进入，不再轮换四个入口；保留往返移动与随分数提升的速度曲线
- 首块出现后即可点击落下，与后续方块使用相同的重叠和落空判定；落下过程中重复点击不会重复落块。
- 实时重叠裁切、切片坠落、失败判定
...
- 体力初始 5 点，自然恢复上限 5 点，广告奖励可持续累积、无上限；不足 5 点时固定每 30 分钟恢复 1 点，跨日不重置，设置中的“看视频 · 恢复体力”在广告完成后增加 5 点，无上限，离线期间照常计算，满体力不囤积恢复时间。每次开始或重新开始正常游戏消耗 1 点，暂停继续和完美测试不消耗；体力不足时点击开始或重开会打开广告二维码，关闭或失败不恢复体力。首页显示体力与下次恢复倒计时，本机存档键为 `wxstack-stamina-v1`，不影响金币、成绩与设置。
- 失败后可「看视频 · 复活」：手机扫码完整观看广告，状态确认完成后自动回到游戏继续；保留分数及下方塔层，塔顶和新方块的宽、深分别至少恢复到初始尺寸的 50%，超过 50% 的尺寸保持不变，每局一次、不额外扣体力。广告等待上限 3 分钟，关闭后不发奖。[广告配置与验证](docs/reward-ads.md)
- 固定使用「奶油积木」风格，Editor、浏览器预览和构建版本共享奶油主题；设置中的“画面风格”可切换标准/明亮两个版本（默认标准），主要改变八色方块与托盘的颜色和材质亮度，保留倒角、阴影及玩法。选择立即生效并保存到 `wxstack-cream-appearance-v1`；不提供皮肤购买。启动时仅尝试清理 `wxstack-selected-skin` 和 `wxstack-owned-skins` 两个旧键，保留金币、最高分、排行榜、昵称、音效和动效设置；即使清理失败，旧主题值也不参与渲染。
- 奶油风格参考粉彩玩具台：八色循环倒角积木、奶油台面与粉色厚边、周边装饰小积木、暖粉背景及浅色首页/HUD/排行榜。台面、塔座和六个周边小积木均具有静态碰撞，脱离塔身的碎块与失败块使用连续碰撞检测。阴影、表面装饰线及 HUD 不参与实体碰撞，镜头为底座留出显示空间。原版资源设计归档见 [极简叠境设计说明](design/skins/minimal-stack.md) 和 [反应堆参考设计](design/skins/reactor-reference-v2.md)。
...
测试使用 Creator 自带的 TypeScript 编译器和 Cocos 测试替身，覆盖响应式布局、焦点与文本对比度、存储迁移、首帧加载、受控落块、无效坐标恢复、重开清场、材质及碎块回收；不代替浏览器中的 Ammo 与 GPU 验证。Creator 安装在其他位置时，设置 `COCOS_CREATOR_APP` 为应用完整路径。
...
塔层落稳后固定为静态刚体，保证经典裁切堆塔玩法稳定；移动块及其 90 毫秒落下过程使用运动学刚体和受控动画，按与目标塔层在 X/Z 两轴的重合范围结算，不依赖物理碰撞回调或接触高度。切片和失败块切换为动态刚体，自由翻转坠落。这不是整塔可自由倾倒的平衡模拟。
